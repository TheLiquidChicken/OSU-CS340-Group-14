...
-- This is the DDL from step 3 with a stored procedure wrapper.
-- This will reset the schema back to the original state.
-- CALL sp_load_3designs_db();

DROP PROCEDURE IF EXISTS sp_load_3designs_db;
--
DELIMITER //
CREATE PROCEDURE sp_load_3designs_db()
BEGIN
    ...
    CREATE TABLE PartMaterials (
      partMaterialID INT NOT NULL AUTO_INCREMENT,
      parts_partID INT NOT NULL,
      materials_materialID INT NOT NULL,
      PRIMARY KEY (partMaterialID),
      UNIQUE (partMaterialID),
    ...
    CREATE TABLE PartMachines (
      partMachineID INT NOT NULL AUTO_INCREMENT,
      parts_partID INT NOT NULL,
      machines_machineID INT NOT NULL,
      PRIMARY KEY (partMachineID),
      UNIQUE (partMachineID),
      FOREIGN KEY (parts_partID) 
        REFERENCES Parts(partID)
        ON DELETE CASCADE,
